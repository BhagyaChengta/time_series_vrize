...
from app.models.modelling_models.models import PreprocessingColumnsMapping, HyperParameterTuneModel,TrainModel
...
class Modelling:
    ...
    def upload_main_file(self):
        ...
        try:
            ...
            input_bucket_name = self.s3_output_location.replace(
                "s3://", '').split('/')
            input_bucket_name = [x for x in input_bucket_name if x.strip() != '']
            folder = 'data_modelling_scripts/sagemaker_script_model_training.py'
            bucket_name = input_bucket_name[0]
            if len(input_bucket_name) > 1:
                folder = "/".join(input_bucket_name[1:]) + \
                    '/data_modelling_scripts/sagemaker_script_model_training.py'
            aws_session = AWS().get_boto3_session()

            s3 = aws_session.client('s3')

            result = s3.upload_file(
                Filename= os.path.join(os.getcwd(),'app','controller','modelling','sagemaker_script_model_training.py'),
                Bucket=bucket_name,
                Key=folder
                )  
    ...
    def run_pipeline(self):
        ...
        try:
            ...
            spark_processor = PySparkProcessor(
                base_job_name="script-model-training",
